...
REQUIREMENTS_FILE = PROJECT_ROOT / "PRODUCT_REQUIREMENTS.md"
...
# 2. Verificar documento de requisitos del producto
if not REQUIREMENTS_FILE.exists():
    requirements_script = PROJECT_ROOT / ".agents" / "skills" / "agent-init" / "scripts" / "generate_requirements.py"
    if requirements_script.exists():
        print("PRODUCT_REQUIREMENTS.md no encontrado. Generando documento base...")
        subprocess.run(
            [sys.executable, str(requirements_script), str(PROJECT_ROOT)],
            check=True,
        )
    else:
        print("WARN: no se pudo generar PRODUCT_REQUIREMENTS.md")

# 3. Verificar DESIGN.md incluso si la configuración ya existía
...
# 5. Generar mapa si no existe o tiene más de 2 horas (7200 segundos)
regenerate = True
if MAP_FILE.exists():
    try:
